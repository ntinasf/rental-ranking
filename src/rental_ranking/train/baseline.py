...
**Baseline A — rank by ``number_of_reviews`` within group.** The label is substantially
establishment-driven, so this is expected to be *strong*, which is the point of freezing it: a
...
**Baseline B — the price + rating heuristic**: rank by good rating and low price, the naive "best
value" ordering a product team would ship on day one. Two construction choices keep it **strong
rather than convenient**, since a weak baseline flatters the model:

* **Within-group percentile ranks, not raw values or z-scores.** Price is heavily right-skewed,
  so a z-score would be dominated by a handful of listings; a percentile is robust and already
  group-local, which is the right frame for a metric computed inside a group.
* **``rating_shrunk`` rather than the raw ``review_scores_rating``.** The raw score ties a third
  of reviewed listings at exactly 5.0 and is null for every listing never reviewed, so a baseline
  built on it would be crippled by ties and missingness — and beating a crippled baseline proves
  nothing.

Neither baseline reads the label and neither is fitted, so there is nothing here to overfit —
which is why the numbers can be recorded before a split exists.

Pure ``DataFrame -> Series`` transforms, no I/O.
"""
...
def rank_by_reviews(listings: pd.DataFrame) -> pd.Series:
    ...
    Deliberately the plainest thing that could work, and expected to be strong because the label
    is establishment-driven. If it is, that is the finding.
...
def rank_by_price_and_rating(
    ...
    Raises:
        ...
        ValueError: If ``price`` or ``rating_shrunk`` is null — both are complete by construction
            in the feature table, so a null means the wrong frame was passed.
    """
    ...
    if nulls.any():
        raise ValueError(
            ...
            "`price` is imputed upstream and `rating_shrunk` returns the city prior at n=0, "
